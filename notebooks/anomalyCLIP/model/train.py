    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        for items in tqdm(train_dataloader):
            ...
            image_loss = F.cross_entropy(
                text_probs.squeeze(), label.long().cuda()
            )
        ...
        if (epoch + 1) % args.save_freq == 0:
            ckp_path = os.path.join(
                args.save_path, "epoch_" + str(epoch + 1) + ".pth"
            )
            torch.save(
                {"prompt_learner": prompt_learner.state_dict()}, ckp_path
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--print_freq", type=int, default=1, help="print frequency"
    )
    parser.add_argument(
        "--save_freq", type=int, default=1, help="save frequency"
    )
