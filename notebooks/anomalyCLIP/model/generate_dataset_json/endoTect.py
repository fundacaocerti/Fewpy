        ...
        for cls_name in self.CLSNAMES:
            ...
            for phase in ["test"]:
                ...
                for specie in species:
                    ...
                    mask_names = (
                        os.listdir(f"{cls_dir}/masks/") if is_abnormal else None
                    )
                    img_names.sort()
                    mask_names.sort() if mask_names is not None else None
                    assert (
                        len(img_names) == len(mask_names)
...
if __name__ == "__main__":
    runner = HyperSolver(
        root="/remote-home/iot_zhouqihang/data/medical/EndoTect_2020_Segmentation_Test_Dataset"
    )
    runner.run()
