...
import fewpy.models.fptrans.base.vit_utils as utils 
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        Args:
            ...
            representation_size (cfgional[int]): enable and set representation layer (pre-logits) to this value if set
        ...
        self.patch_embed = embed_layer(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, stride=args.vit_stride)
        num_patches = self.patch_embed.num_patches

        # prompt tokens
        self.prompt_tokens = None
        if self.allow_mod:
            ncls = 15 if args.dataset == "pascal" else 60
            if args.coco2pascal:
                ncls = 60
            divider = 1 + args.bg_num * args.kshot
            self.prompt_tokens = nn.Parameter(
                torch.zeros(ncls * divider, args.num_prompt // divider, embed_dim))  # [bank_size, G, embed_dim]
            nn.init.normal_(self.prompt_tokens.permute(2, 0, 1), std=args.pt_std)
    ...
    def forward(self, x):
        ...
        B = x.shape[0] // (1 + self.args.kshot)
        divider = 1 + self.args.bg_num * self.args.kshot
...
vit_factory = {
    ...
    'VIT-B':              {'patch_size': 16, 'embed_dim':  768, 'depth': 12, 'num_heads': 12, 'distilled': False},
    ...
    'DEIT-T':         {'patch_size': 16, 'embed_dim':  192, 'depth': 12, 'num_heads': 3, 'distilled': True},
    'DEIT-S':         {'patch_size': 16, 'embed_dim':  384, 'depth': 12, 'num_heads': 6, 'distilled': True},
   #'DeiT-B/16':  {'patch_size': 16, 'embed_dim':  768, 'depth': 12, 'num_heads': 12, 'distilled': True},
    'DEIT-B':     {'patch_size': 16, 'embed_dim':  768, 'depth': 12, 'num_heads': 12, 'distilled': True},
}
...
def vit_model(model_type,
    ...
    return VisionTransformer(img_size=image_size,
                             ...
                             depth=depth or args.vit_depth or vit_factory[model_type]['depth'],
